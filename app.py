...
# --- PDF GENERATOR LOGIK (Korrigiert für fpdf2) ---
...
def create_pdf(name, adresse, steuernummer, finanzamt, datum_bescheid, text, betreff):
    ...
    # KORREKTUR HIER: Einfaches .output() für fpdf2
    return pdf.output()
...
try:
    ...
    st.error(f"Verbindungsfehler Setup: {e}")
...
if "Grundsteuer" in fall:
    betreff, text = "Einspruch gegen Grundsteuerwert", "hiermit lege ich Einspruch ein, da die Bodenrichtwerte zweifelhaft sind."
elif "Krypto" in fall:
    betreff, text = "Einspruch gegen ESt-Bescheid (Krypto)", "hiermit lege ich Einspruch ein, da die Haltefrist von einem Jahr gemäß § 23 EStG nicht korrekt berücksichtigt wurde."
else:
    betreff, text = "Einspruch gegen Steuerbescheid", "hiermit lege ich fristwahrend Einspruch ein."
    ...
    if u_name and u_snr:
        ...
        if conn:
            try:
                ...
                st.warning(f"Statistik konnte nicht gespeichert werden (Fehler: {e}), aber Ihr PDF wird trotzdem erstellt.")
        
        # PDF Erstellung
        try:
            pdf_data = create_pdf(u_name, u_adresse, u_snr, u_fa, u_datum, text, betreff)
            st.download_button("Datei jetzt speichern", data=pdf_data, file_name="Einspruch.pdf", mime="application/pdf")
...
st.markdown(f"*„Ich arbeite beim Finanzamt und ärgere mich täglich, wie so viele Menschen Geld auf der Straße liegen lassen...“*")
